...
namespace SCS.HomePhotos.Data.Core
{
    ...
    /// </summary>
    public class UserTokenData : DataBase<UserToken>, IUserTokenData
    {
        /// <summary>
        ...
        {
            return await GetListAsync("WHERE UserId = @UserId AND Refresh = @Refresh",
        ...
        {
            var userTokens = await GetListAsync("WHERE UserId = @UserId AND Refresh = @Refresh AND Token = @Token",
        ...
        {
            var userTokens = await GetListAsync("WHERE UserId = @UserId AND Refresh = @Refresh AND AgentIdentifier = @AgentIdentifier",
