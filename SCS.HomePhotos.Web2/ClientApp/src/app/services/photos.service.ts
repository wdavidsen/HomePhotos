...
export class PhotosService {
    ...
    searchPhotos(pageNum: number, searchInfo: SearchInfo) {
        ...
        return this.http.get<Photo[]>(`${environment.apiUrl}/photos/search?pageNum=${pageNum}&keywords=${keywords}&username=${searchInfo.username ?? ''}&fromDate=${searchInfo.fromDate ?? ''}&toDate=${searchInfo.toDate ?? ''}`);
    }
